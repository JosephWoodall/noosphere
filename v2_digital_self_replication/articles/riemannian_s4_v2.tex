...
Second, empirical: on PhysioNetMI (20 subjects, 3-class), EEGNet~\cite{lawhern2018eegnet} ($38.4\%$, fully supervised) and the JEPA encoder ($36.6\%$, self-supervised) are statistically indistinguishable ($p=0.46$), neither significantly outperforming random initialisation ($p=0.17$, $p=0.70$), yet both remain $\approx$$22$\,pp below CSP+LDA ($60.1\%$).
Cross-dataset validation on BCI Competition IV Dataset 2a (9 subjects, 4-class) yields an identical pattern: CSP $58.7\%$ vs.\ EEGNet $35.1\%$ ($\Delta=23.6$\,pp, $p=0.004$).
A closed-loop virtual arm simulation ($N=300$ trials) confirms the operational cost: the decoder outputs near-zero DOF commands, yielding 0\,\% target convergence.
We provide a mechanistic explanation — limited labelled data ($\leq\!576$ trials per subject), not the pretraining objective, is the binding constraint — and establish this quantitatively across two independent datasets.
...
A decoder operating at $\approx\!37\%$ balanced accuracy does not reliably map latent EEG states to class-specific DOF directions.
