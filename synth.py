...
class NeckEEGGenerator:
    ...
    def __init__(self, seed: Optional[int] = 42):
        ...
        self._drift_amp  = self.rng.uniform(10., 50., size=3)      # μV
        self._noise_std  = self.rng.uniform(6.0, 10.0, size=3)     # μV (higher than scalp)
        self._muscle_amp = np.array([120., 200., 130.])             # μV (strong at neck)
        ...
        self._blink_amp_neck = np.array([35., 20., 35.])  # μV, symmetric at neck
    ...
    def _next_sample(self) -> np.ndarray:
        ...
        for ch in range(3):
            ...
            if self._blink_phase < 128:
                ...
                if ch == 2: self._blink_phase += 1  # advance once per sample
            if t >= self._next_blink:
                self._blink_phase = 0
                self._next_blink  = t + self.rng.exponential(1.0 / 0.2).clip(0.8, 12.0)
            ...
            if self._muscle_remaining == 0 and self.rng.random() < 0.002:
                ...
                self._muscle_freq = self.rng.uniform(30., 150.)
            if self._muscle_remaining > 0:
                self._muscle_remaining -= 1
                muscle = self._muscle_amp[ch] * math.sin(2*math.pi*self._muscle_freq*t)
            ...
            alpha_scale = 0.3
            alpha = 8.0 * alpha_scale * self._env_alpha[ch] * math.sin(2*math.pi*10.5*t)
    ...
    def next_segment(
        ...
        if intent is not None:
            ...
            self._muscle_freq = self.rng.uniform(30., 120.)
        ...
        if dom == self.LABEL_MUSCLE and var > 1500.0:
            m_intent = self._muscle_intent if intent is not None else \
                       int(self.rng.integers(1, 10))
            kinematic = {
                "x":        float(self.rng.uniform(-1.0, 1.0)),
                "y":        float(self.rng.uniform(-1.0, 1.0)),
                "z":        float(self.rng.uniform(-0.5, 0.5)),
...
def synth_stereo(H: int = 64, W: int = 64, baseline: float = 0.12,
...
def obs_bci(
    ...
    imu = synth_imu(5, 5, seed)   # [cursor_x, cursor_y, target_x, target_y, time_remaining]
    return {
