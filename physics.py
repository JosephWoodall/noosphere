...
class PhysicalState:
    ...
    def __init__(self, B: int, N: int, device: torch.device, G: int = 4):
        ...
        self.rot = torch.zeros(B, N, 4, device=device)
        self.rot[..., 0] = 1.0
    ...
    def flatten(self) -> torch.Tensor:
        ...
        return torch.cat(
            [
                self.pos.reshape(B, -1),
                self.vel.reshape(B, -1),
                self.rot.reshape(B, -1),
                self.omega.reshape(B, -1),
                self.mass.reshape(B, -1),
                self.inertia.reshape(B, -1),
                self.forces.reshape(B, -1),
                self.contacts.reshape(B, -1),
                self.energy.unsqueeze(-1),
                self.fluid_v.reshape(B, -1),
            ],
            dim=-1,
        )
...
class PhysicsStateEstimator(nn.Module):
    def __init__(
        self, embed_dim: int, n_bodies: int = 4, hidden_dim: int = 256, G: int = 4
    ):
        super().__init__()
        self.N = n_bodies
        self.G = G
...
class PhysicsTransitionPrior(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "_g_vec", torch.tensor([0.0, 0.0, -gravity], dtype=torch.float32)
        )
    ...
    def _deriv(self, s: PhysicalState, Fext: torch.Tensor, tau: torch.Tensor):
        ...
        pi = s.pos.unsqueeze(2)
        pj = s.pos.unsqueeze(1)
        d_ij = pj - pi
        dist = d_ij.norm(-1, keepdim=True).clamp(1e-6)
        n_hat = d_ij / dist
        pen = F.relu(1.0 - dist.squeeze(-1))
        vi = s.vel.unsqueeze(2)
        vj = s.vel.unsqueeze(1)
        Fc = (
    ...
    def _rk4_apply(self, s, dt, dp, dv, dr, dw, de, dfluid):
        G = int(round(s.fluid_v.shape[1] ** (1 / 3)))
    ...
    def forward(self, s: PhysicalState, Fext: torch.Tensor, tau: torch.Tensor):
        ...
        k2 = self._deriv(self._rk4_apply(s, dt / 2, *k1), Fext, tau)
        k3 = self._deriv(self._rk4_apply(s, dt / 2, *k2), Fext, tau)
        k4 = self._deriv(self._rk4_apply(s, dt, *k3), Fext, tau)

        def rk4c(y, *ks):
            return y + (dt / 6) * (ks[0] + 2 * ks[1] + 2 * ks[2] + ks[3])
...
class ConservationLaws(nn.Module):
    def __init__(
        self, energy_w=1.0, momentum_w=1.0, angular_w=0.5, quat_w=1.0, fluid_w=0.5
    ):
        super().__init__()
        self.ew = energy_w
        self.pw = momentum_w
    ...
    def forward(self, s0, s1, Fext, tau, dt) -> Dict[str, torch.Tensor]:
        ...
        dp_exp = (
            Fext
            + s0.mass.unsqueeze(-1)
            * torch.tensor([0.0, 0.0, -9.81], device=Fext.device, dtype=Fext.dtype)
        ).sum(1) * dt
