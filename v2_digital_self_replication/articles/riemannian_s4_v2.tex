...
Fig.~\ref{fig:overview} provides an at-a-glance architecture overview; Figs.~\ref{fig:pipeline} and~\ref{fig:architecture} detail the full inference pipeline and the encoder layer stack respectively.

%% ── Compact single-column overview (quick-read) ──────────────────────────────
\begin{figure}[t]
\centering
...
\begin{tikzpicture}[
  ...
  block/.style={rectangle, draw, rounded corners=2pt,
    fill=blue!12, minimum width=1.5cm, minimum height=0.75cm,
    text centered, text width=1.45cm},
  novel/.style={rectangle, draw, line width=1.6pt, rounded corners=2pt,
    fill=orange!28, minimum width=1.5cm, minimum height=0.75cm,
    text centered, text width=1.45cm},
  io/.style={rectangle, draw, rounded corners=2pt,
    fill=cyan!25, minimum width=1.25cm, minimum height=0.68cm,
    text centered, text width=1.2cm},
  outbox/.style={rectangle, draw, rounded corners=2pt,
    fill=green!22, minimum width=1.35cm, minimum height=0.58cm,
    text centered, text width=1.3cm, font=\scriptsize},
  side/.style={rectangle, draw, rounded corners=2pt,
    fill=orange!22, minimum width=1.15cm, minimum height=0.58cm,
    text centered, text width=1.1cm, font=\scriptsize},
  arr/.style={-{Stealth[length=2.5mm]}, thick},
]
%% Left-to-right data flow
\node[io]     (inp)  at (0.0,  0.0) {EEG\\$T{\times}21$};
\node[novel]  (b1)   at (2.0,  0.0) {\textbf{AC-SSM}\\Block~1};
\node[block]  (b24)  at (4.0,  0.0) {SSM\\Blocks 2--4};
\node[block]  (pool) at (6.0,  0.0) {Temporal\\Pool};
\node[block]  (dec)  at (8.0,  0.0) {Intent\\Decoder};
%% Output heads
\node[outbox] (mu)   at (10.3,  0.58) {$\bm\mu$: 6-DOF};
\node[outbox] (sig)  at (10.3,  0.00) {$\bm\sigma$: uncert.};
\node[outbox] (ern)  at (10.3, -0.58) {$p_\text{ERN}$: ERN};
%% Previous motor command
\node[side]   (act)  at (2.0,  -1.45) {$\mathbf{a}_{t-1}$\\prev.\,cmd};
%% Arrows
\draw[arr] (inp)  -- (b1);
\draw[arr] (b1)   -- (b24);
\draw[arr] (b24)  -- (pool);
...
\draw[arr] (act.north) --
    node[right=1mm, font=\tiny]{SiLU} (b1.south);
\end{tikzpicture}%
}%
\caption{Architecture overview (inference path).
\textbf{Orange} (thick border): novel action-conditioned Block~1, where the
previous motor command $\mathbf{a}_{t-1}$ enters via a SiLU gate.
\textbf{Blue:} standard ZOH-SSM blocks (2--4).
\textbf{Cyan:} EEG input tensor.
\textbf{Green:} three decoder heads ($\bm\mu$, $\bm\sigma$, $p_\text{ERN}$).
Full pipeline: Fig.~\ref{fig:pipeline}; layer detail: Fig.~\ref{fig:architecture}.}
\label{fig:overview}
\end{figure}
...
\begin{tikzpicture}[
  every node/.style={font=\small},
  block/.style={rectangle, draw, rounded corners=2pt,
                minimum width=1.7cm, minimum height=0.72cm,
                text centered, text width=1.6cm, fill=blue!8},
  newblock/.style={rectangle, draw, rounded corners=2pt,
                   minimum width=1.7cm, minimum height=0.72cm,
                   text centered, text width=1.6cm, fill=orange!18, thick},
  outblock/.style={rectangle, draw, rounded corners=2pt,
                   minimum width=2.5cm, minimum height=0.62cm,
                   text centered, fill=gray!7},
  arr/.style={-{Stealth[length=2mm]}, thick},
  darr/.style={-{Stealth[length=2mm]}, thick, dashed, gray},
]

%% Absolute-coordinate layout eliminates all arrow crossings.
%% x-columns: 0 (inputs), 2.4 (fusion), 4.9 (encoder), 7.4 (latent),
%%            9.8 (decoder/transition), 13.0 (outputs)
%% y-rows:    1.2 / 0 / -1.3 (inputs); 1.6 (JEPA);
%%            0.9 (decoder); -1.1 (transition); -2.6 (planner);
%%            -4.1 (Kalman); -5.5 (safety); -6.9 (hardware)

%% ── Input modalities ─────────────────────────────────────────────────────
\node[block] (eeg)  at (0,  1.2) {EEG\\(21\,ch)};
\node[block] (hrv)  at (0,  0.0) {HRV/GSR};
\node[block] (prop) at (0, -1.3) {Prop.\ (6\,DOF)};

%% ── Fusion ───────────────────────────────────────────────────────────────
\node[block] (fuse) at (2.4, 0.0) {Modal\\Fusion};
\draw[arr] (eeg.east)  -- ++(4mm,0) |- (fuse.north west);
\draw[arr] (hrv.east)  -- (fuse.west);
\draw[arr] (prop.east) -- ++(4mm,0) |- (fuse.south west);

%% ── Encoder ──────────────────────────────────────────────────────────────
\node[block] (enc) at (4.9, 0.0) {ZOH-SSM\\$\times$4 layers};
\draw[arr] (fuse) -- (enc);

\node[outblock] (jepa) at (4.9, 1.6) {JEPA\\pretrain};
\draw[darr] (enc.north) -- (jepa.south)
    node[midway, right=1.5mm, gray, font=\tiny] {offline};

%% ── Latent ───────────────────────────────────────────────────────────────
\node[block] (lat) at (7.4, 0.0) {Latent $\mathbf{h}$\\(128-d)};
\draw[arr] (enc) -- (lat);

%% ── Intent Decoder (upper path) ──────────────────────────────────────────
\node[block] (dec) at (9.8, 0.9) {Intent\\Decoder};
\draw[arr] (lat.north east) -- ++(3mm,0) |- (dec.west);

%% Output nodes in their own column -- no arrows need to cross back left
\node[outblock] (mu)    at (13.0,  1.4) {$\bm\mu$: 6-DOF cmd};
\node[outblock] (sigma) at (13.0,  0.9) {$\bm\sigma$: uncertainty};
\node[outblock] (ern)   at (13.0,  0.4) {$p_\text{ERN}$: error sig.};
\draw[arr] (dec.east) -- ++(3mm,0) |- (mu.west);
\draw[arr] (dec.east) --            (sigma.west);
\draw[arr] (dec.east) -- ++(3mm,0) |- (ern.west);

%% ── Action-Conditioned Transition (lower path) ───────────────────────────
\node[newblock] (trans) at (9.8, -1.1) {Action-Cond.\\ Transition $T$};
\draw[arr] (lat.south east) -- ++(3mm,0) |- (trans.west);
%% mu feeds the action input to the transition
\draw[darr] (mu.west) -- ++(-3mm,0) -- ++(0,-8mm) -- ++(0,-16mm)
    -| (trans.north) node[near end, right=1mm, gray, font=\tiny] {$\mathbf{a}$};

%% ── Latency Planner ──────────────────────────────────────────────────────
\node[newblock] (plan) at (9.8, -2.6) {Latency\\Planner (CEM)};
\draw[arr] (trans.south) -- (plan.north);

%% ── Kalman Filter ────────────────────────────────────────────────────────
\node[block] (kalman) at (9.8, -4.1) {Kalman\\Filter};
\draw[arr] (plan.south) -- node[right=1mm, font=\tiny] {$\hat{\mathbf{a}}$} (kalman.north);
%% sigma routes: right past output column, then down to kalman's y, then left
\draw[darr] (sigma.east) -- ++(6mm,0) |- (kalman.east);

%% ── Safety Gate ──────────────────────────────────────────────────────────
\node[block] (safety) at (9.8, -5.5) {Safety Gate\\(ERN, $\bm\sigma$)};
\draw[arr] (kalman) -- (safety);
%% ern routes: right past output column, then down to safety's y, then left
\draw[darr] (ern.east) -- ++(6mm,0) |- (safety.east);

%% ── Hardware ─────────────────────────────────────────────────────────────
\node[block] (hw) at (9.8, -6.9) {Hardware\\(6-DOF arm)};
\draw[arr] (safety) -- (hw);

%% ── Legend ───────────────────────────────────────────────────────────────
\node[block,    minimum width=0.9cm, minimum height=0.4cm, text width=0.9cm]
    (l1) at (0, -5.5) {};
\node[right=2mm of l1, font=\footnotesize, anchor=west]
    {ZOH-SSM + JEPA (prior work)};
\node[newblock, minimum width=0.9cm, minimum height=0.4cm, text width=0.9cm]
    (l2) at (0, -6.3) {};
\node[right=2mm of l2, font=\footnotesize, anchor=west]
    {World-model additions (new)};

\end{tikzpicture}%
}%
\caption{ZOH-SSM World Model inference pipeline.
Blue blocks: pretrained encoder-decoder (ZOH-SSM + JEPA objective).
Orange blocks: novel world-model components (Action-Conditioned Transition $T(\mathbf{h},\mathbf{a})$
and hybrid CEM+gradient Latency Planner).
Gray output nodes: decoder outputs fed to downstream components.
Dashed arrows indicate offline pretraining or uncertainty signal routing.
In real-time BCI operation the fast self-conditioning path
$\mathbf{h}_\text{plan} = T(\mathbf{h}, \mathrm{dec}(\mathbf{h}).\bm\mu)$
replaces the full CEM optimisation loop ($<$5\,ms CPU).}
\label{fig:pipeline}
\end{figure*}
...
path and closed-loop CEM planner are shown in Fig.~\ref{fig:pipeline}.}
...
\midrule
S01 & n/a & 43.3 & 29.0 & 27.7 & 29.0 & 78.2 & 66.5 \\
S02 & n/a & 42.0 & 40.0 & 30.3 & 34.7 & 71.9 & 63.7 \\
S03 & n/a & 36.7 & 38.3 & 31.3 & 38.7 & 49.5 & 61.1 \\
S04 & n/a & 43.3 & 37.3 & 41.7 & 46.7 & 68.1 & 55.1 \\
S05 & n/a & 35.7 & 33.7 & 39.7 & 41.7 & 63.6 & 46.3 \\
S06 & n/a & 35.7 & 32.3 & 43.0 & 34.3 & 63.6 & 36.3 \\
S07 & n/a & 35.7 & 31.7 & 33.0 & 31.0 & 83.7 & 63.3 \\
S08 & n/a & 37.0 & 35.0 & 33.3 & 30.0 & 57.1 & 54.0 \\
S09 & n/a & 29.7 & 23.3 & 32.0 & 24.7 & 50.8 & 39.6 \\
S10 & n/a & 29.3 & 33.0 & 22.3 & 40.0 & 63.8 & 44.6 \\
S11 & n/a & 45.0 & 41.0 & 36.7 & 43.7 & 57.4 & 43.2 \\
S12 & n/a & 48.7 & 41.7 & 45.7 & 43.3 & 58.6 & 56.3 \\
S13 & n/a & 29.3 & 37.7 & 33.7 & 33.3 & 50.8 & 28.5 \\
S14 & n/a & 31.7 & 45.3 & 30.7 & 34.3 & 62.1 & 53.8 \\
S15 & n/a & 28.0 & 40.7 & 26.7 & 33.3 & 84.2 & 59.1 \\
S16 & n/a & 41.7 & 33.0 & 40.7 & 43.0 & 49.0 & 33.5 \\
S17 & n/a & 30.0 & 27.0 & 21.3 & 28.7 & 42.9 & 34.7 \\
S18 & n/a & 36.0 & 27.0 & 36.7 & 34.0 & 58.1 & 34.9 \\
S19 & n/a & 33.3 & 31.7 & 28.3 & 38.0 & 48.1 & 35.0 \\
S20 & n/a & 38.0 & 35.7 & 32.3 & 31.0 & 41.3 & 40.1 \\
\midrule
\textbf{Mean} & \textbf{n/a} & \textbf{36.5} & \textbf{34.7} & \textbf{33.3} & \textbf{35.7} & \textbf{60.1} & \textbf{47.5} \\
\textbf{95\,\% CI} & n/a & [34.1,39.2] & [32.4,37.2] & [30.5,36.5] & [33.2,38.2] & [54.7,65.6] & [42.5,52.8] \\
\bottomrule
\end{tabular}
\par\vspace{2pt}
\begin{minipage}{\linewidth}
{\footnotesize\raggedright EEGNet = EEGNet (Lawhern 2018)~\cite{lawhern2018eegnet};
Enc = encoder\_ft\_cls; AC-SSM = action-conditioned SSM (this work);
Enc+WM = MLP world-model planner; Abl = ablation; CSP = CSP+LDA; MDM = Riemannian MDM.}
\end{minipage}
\end{table*}
