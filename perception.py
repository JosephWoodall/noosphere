...
class ResidualInjection(nn.Module):
    ...
    def forward(
        self, tokens: torch.Tensor, ext: torch.Tensor, pos: int
    ) -> torch.Tensor:
        e = self.proj(ext)
...
class HybridBlock(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        cross_s4: bool = False,
        cross_gnn: bool = False,
    ):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.sattn = nn.MultiheadAttention(
            d_model, n_heads, dropout=dropout, batch_first=True
        )
        ...
        self.drop = nn.Dropout(dropout)
        self.xattn_s4 = (
            GatedCrossAttention(d_model, n_heads, dropout) if cross_s4 else None
        )
        self.xattn_gnn = (
            GatedCrossAttention(d_model, n_heads, dropout) if cross_gnn else None
        )
    ...
    def forward(
        ...
        x = x + self.drop(h)
        if self.xattn_s4 and s4_seq is not None:
            x = self.xattn_s4(x, s4_seq)
        if self.xattn_gnn and gnn_nodes is not None:
            x = self.xattn_gnn(x, gnn_nodes)
        if s4_summary is not None:
            x = self.inject_s4(x, s4_summary, pos=1)
        if gnn_summary is not None:
            x = self.inject_gnn(x, gnn_summary, pos=2)
...
class HybridPerceptionModel(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                HybridBlock(
                    ...
                    d_model * 4,
                    dropout,
                    cross_s4=(l in self._cross),
                    cross_gnn=(l in self._cross),
                )
    ...
    def forward(
        ...
        if has_vis:
            try:
                ...
                pad_mask = pad_mask[:, 1:]  # strip corresponding CLS mask entry
            except Exception:
                pass
        ...
        if vis_tokens is not None:
            ...
            if pad_mask is not None:
                ...
                mask_parts.append(
                    torch.zeros(B, vis_tokens.shape[1], dtype=torch.bool, device=dev)
                )
        ...
        pad_mask = (
            ...
            if any(m.any().item() for m in mask_parts)
            else None
        )
    ...
    def print_profile(self):
        ...
        print(
            f"  {'TOTAL':<22} {total:6.2f}ms  ({total / 50 * 100:.0f}% of 50ms budget)"
        )
        print("──────────────────────────────────────────────────\n")
