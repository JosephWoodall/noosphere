...
The 22.7\,pp gap between CSP+LDA and the JEPA encoder ($60.1\%$ vs.\ $37.4\%$) quantifies exactly how much discriminative information JEPA's objective fails to preserve, despite providing a significant $+4.1$\,pp improvement over random initialisation.
...
\begin{enumerate}
  \item \textbf{Single dataset.} All results are from PhysioNetMI (20 subjects). Replication on BCI Competition IV Dataset 2a~\cite{brunner2008bci} (9 subjects, 4-class, standard benchmark) and/or BCI Competition IV Dataset 2b would establish generalisability across recording conditions and classification demands. This is the most important strengthening for a follow-up submission.
...
A rigorous seven-condition LOSO evaluation across 20 PhysioNetMI subjects shows that JEPA pretraining provides a significant but modest benefit: $37.4\%$ balanced accuracy ($p{<}0.001$ vs.\ chance; $p=0.011$ vs.\ random initialisation), yet $22.7$\,pp below CSP+LDA ($60.1\%$).
World-model self-conditioned classification ($33.7\%$) is significantly below the base encoder ($p=0.008$), confirming that static accuracy is the wrong metric for a system designed to minimise closed-loop latency.
We show the discriminability gap is structurally necessary — JEPA optimises temporal predictability, not class discriminability — and that the world model's value lies in closed-loop latency reduction rather than static accuracy improvement.
