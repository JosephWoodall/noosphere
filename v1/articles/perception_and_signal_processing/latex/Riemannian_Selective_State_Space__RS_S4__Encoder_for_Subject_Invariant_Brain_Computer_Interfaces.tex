...
sample covariance is computed as:
\begin{equation}
    \mathbf{C} = \frac{1}{T-1} \mathbf{X}\mathbf{X}^\top
\end{equation}
These matrices reside on the Symmetric Positive Definite (SPD) manifold
$\mathcal{M}(C)$.
A Riemannian Stem maps each trial to a tangent-space feature vector by
computing the matrix logarithm $\log(\mathbf{C})$ and extracting its
upper-triangular elements (dimensionality $C(C+1)/2$), projecting to a
$d$-dimensional embedding via a learned linear layer followed by GELU and
LayerNorm.
...
(LOSO).
The whitening transform $\mathbf{R} = \bar{\mathbf{P}}^{-1/2}$ pre-aligns
signals before covariance estimation:
\begin{equation}
...
Training applies Riemannian Mixup ($\alpha=0.2$) in tangent space to improve
generalization across subjects.

\subsubsection{Multi-Scale Wavelet Temporal Stem}

A parametric Morlet wavelet bank decomposes each aligned EEG trial into
high-frequency ($\gamma$/$\beta$ bands; kernel sizes 3, 7 samples) and
low-frequency ($\alpha$/$\theta$/$\delta$ bands; kernel sizes 15, 31, 63
samples) feature streams.
Each Morlet kernel is parameterized by a learnable center frequency and
bandwidth:
\begin{equation}
    \psi(t; f, \sigma) = \cos(2\pi f t)\,
    \exp\!\left(-\frac{t^2}{2\sigma^2}\right)
\end{equation}
The two streams are independently projected to $d$ dimensions via 1$\times$1
convolutions followed by GELU and GroupNorm, yielding sequences
$\mathbf{x}_\text{high}, \mathbf{x}_\text{low} \in \mathbb{R}^{L \times d}$.
The Riemannian stem token $\mathbf{x}_s \in \mathbb{R}^{1 \times d}$ is
prepended to each stream, providing a global spatial context token.

\subsubsection{Selective State Space Backbone}

Each stream is processed by $n_\text{blocks}/2$ Selective S4 blocks.
Each block implements a data-dependent structured state space model inspired
by Mamba~\cite{Gu2023Mamba}:
\begin{align}
    \Delta_t &= \text{softplus}(W_\Delta \mathbf{x}_t)\cdot
                \sigma(\mathbf{x}_t \odot \boldsymbol{\tau}) \\
    \mathbf{B}_t &= W_B\,\mathbf{x}_t, \quad
    \mathbf{C}_t = W_C\,\mathbf{x}_t
\end{align}
where $\boldsymbol{\tau}$ is a learned Liquid Time-Constant vector and
$\sigma$ is sigmoid.
The input-dependent step size $\Delta_t$ enables the model to selectively
retain or discard state depending on signal content, a critical property for
non-stationary EEG.
State evolution follows a symplectic (Störmer-Verlet) leapfrog scheme over
a Hamiltonian decomposition of the state vector, maintaining long-horizon
stability without ZOH discretization overhead.
Post-scan LayerNorm prevents state accumulation overflow across long sequences.
A FiLM~\cite{Perez2018} conditioning layer modulates each block's output using
the Riemannian spatial token, injecting subject geometry at every temporal
processing stage.
Blocks output through a gated linear unit (GLU) with residual connection.

\subsubsection{Attention Pooling and Intent Decoding}

A Multi-Head Attention Pooling layer collapses the $L$-length sequence to a
fixed representation using a single learned query vector $\mathbf{q}$:
\begin{equation}
    \mathbf{z} = \mathrm{LayerNorm}\!\left(
        \mathrm{MHA}(\mathbf{q}, \mathbf{X}, \mathbf{X})
    \right)
\end{equation}
This provides a content-adaptive global pooling that attends to the most
task-relevant temporal positions.
The pooled representation $\mathbf{z}$ is passed to a Dirichlet Evidential
Deep Learning head~\cite{Sensoy2018}: a two-layer MLP with SiLU activations
and a LeakyIntegrate-and-Fire output layer produces non-negative evidence
values, from which Dirichlet $\alpha$ parameters are derived as
$\boldsymbol{\alpha} = \mathbf{e} + 1$.
This yields calibrated uncertainty estimates alongside predictions.
Training uses label-smoothed negative log-likelihood ($\epsilon = 0.1$) with
per-class weighted sampling to handle class imbalance.
Post-hoc temperature scaling ($T = 1.5$) was applied before inference to
further improve probability calibration.
